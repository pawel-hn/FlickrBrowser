...
abstract class CoroutinesAsyncTask<Params, Progress, Result>{
    ...
    var status: AsyncTask.Status = AsyncTask.Status.PENDING
    ...
    fun execute(vararg params: Params){
        ...
        if (status != AsyncTask.Status.PENDING) {
            when (status) {
                AsyncTask.Status.RUNNING -> throw IllegalStateException("Cannot execute task:" + " the task is already running.")
